...
function begin_play(){
	...
	if (player_1 == "" || player_2 == "") { // if any of the plyers name filed is empty, give the users an alrt mssege. 
		alert("Two players game, both fields are mondetory.");
		return false;
	}
	if(!started){ // if the game not strted yet disable both plyer
             document.getElementById("player1_id").disabled = true;
             document.getElementById("player2_id").disabled = true;
             this.started = true; // turn strated to true.
...
*/
function reset_play(){ // if game reset it make the player name empty. and return started to false. 
	...
	document.getElementById("turn_info").innerHTML = "Game hasn't start";
	this.started = false;
	this.turn = 1;
	for (var i = 0; i < table_ids.length; i++) { // go throu the table and turn all index to -1.
...
*/
function play() {
	  if (started) { // truck player moves.
         var plyerTurn = document.getElementById("move_text_id").value;
         if (!table_ids.includes(plyerTurn)) {
             alert("Invalid move.");
         } else {
             if(this.board_state[this.table_ids.indexOf(plyerTurn)] == -1)
             {
                 if (whose_move()) {
                     document.getElementById(plyerTurn).innerHTML = "X";
                     this.board_state[this.table_ids.indexOf(plyerTurn)] = 1;
                     document.getElementById("turn_info").innerHTML = "Turn For: O" ;
                     toggle_move();
                 } else {
                     document.getElementById(plyerTurn).innerHTML = "O";
                     board_state[table_ids.indexOf(plyerTurn)] = 0;
                     document.getElementById("turn_info").innerHTML="Turn For: X";
                     toggle_move();
                 }         
             }else{ // if the player chooes a full cell alrat them. 
